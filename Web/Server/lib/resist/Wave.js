module.exports = Wave = function (playerList, gameWidth, gameHeight, unitManager) {
	...
	this.digest = function () {
		for (var i = 0; i < this.spriteList.length; i++) {
			if (this.spriteList[i].sprite.destY <= this.spriteList[i].sprite.y && this.spriteList[i].sprite.destY < gameHeight - 100) {
				this.spriteList[i].sprite.destX = Math.floor(Math.random() * this.gameWidth);
				this.spriteList[i].sprite.destY += Math.floor(Math.random() * 30);

				if (this.spriteList[i].sprite.y > gameHeight - 100) {
					this.spriteList[i].sprite.destY = this.spriteList[i].sprite.y;
				}
